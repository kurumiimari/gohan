...
const (
	...
	// Handshake-specific covenant ops
	OP_TYPE = 0xd0 // 208
...
// opcodeBlake160 treats the top item of the data stack as raw bytes and replaces
// it with blake160(data).
//
// Stack transformation: [... x1] -> [... blake160(x1)]
func opcodeBlake160(op *parsedOpcode, vm *Engine) error {
	buf, err := vm.dstack.PopByteArray()
	if err != nil {
		return err
	}
	vm.dstack.PushByteArray(blake160(buf))
	return nil
}

// opcodeBlake256 treats the top item of the data stack as raw bytes and replaces
// it with blake256(data).
//
...
// it with sha3(data).
//
// Stack transformation: [... x1] -> [... sha3(x1)]
...
// it with Keccak(data).
//
// Stack transformation: [... x1] -> [... keccak(x1)]
func opcodeKeccak(op *parsedOpcode, vm *Engine) error {
	buf, err := vm.dstack.PopByteArray()
	if err != nil {
		return err
	}
...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	signature, err := chain.DeserializeSignature(sigBytes)
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
	}
...
func opcodeType(op *parsedOpcode, vm *Engine) error {
	cov := vm.tx.Outputs[vm.txIdx].Covenant.Type
	vm.dstack.PushByteArray([]byte{byte(cov)})
	return nil
}
